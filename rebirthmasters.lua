...
║  v3.4: Nested submenu architecture (4 categories)          ║
║    1. Find Offsets  2. Currency Hacks                       ║
║    3. Auto Hacks    4. Configuration                        ║
╚══════════════════════════════════════════════════════════════╝
]]
...
-- ═══════════════════════════════════════════════════════════════
-- MENU SYSTEM  — nested submenus, no tabs
-- Structure:
--   mainMenu()
--     ├─ 1. Find Offsets
--     │     ├─ Manual Find (sub)
...
local function status_bar()
    ...
    local farm = S.auto_farm and "  🌾 FARMING" or ""
    return sformat("v%s  |  %d/7 found%s", CFG.VER, n, farm)
end

-- ─── helper: ask current value for manual find ───────────────
local function ask_and_find(key)
    local c   = CURR[key]
    local inp = gg.prompt(
        { "Enter your CURRENT " .. c.name .. " shown in-game:" },
        { tostring(c.last > 0 and c.last or 0) },
        { "number" }
    )
    if inp and inp[1] then
        find_obscured(key, tonumber(inp[1]) or 0)
    end
end

-- ═══════════════════════════════════════════════════════════════
-- 1a. FIND OFFSETS → Manual Find
-- ═══════════════════════════════════════════════════════════════
local function menu_find_manual()
    while true do
        local m = gg.choice({
            "💰 Find Gold",
            "🏅 Find Honor",
            ...
            "⬆️  Find Team Level",
            "⬅️  Back",
        }, nil, "FIND OFFSETS » Manual\n" .. status_bar())

        if     m == 1 then find_gold_manual()
        elseif m == 2 then ask_and_find("honor")
        elseif m == 3 then ask_and_find("carat")
        elseif m == 4 then ask_and_find("soulstone")
        elseif m == 5 then ask_and_find("stage")
        elseif m == 6 then ask_and_find("highstage")
        elseif m == 7 then ask_and_find("teamlevel")
        else break
        end
    end
end
...
-- ═══════════════════════════════════════════════════════════════
-- 1b. FIND OFFSETS → Auto Find
-- ═══════════════════════════════════════════════════════════════
local function menu_find_auto()
    while true do
        local m = gg.choice({
            "💰 Auto-Find Gold",
            "🏅 Auto-Find Honor",
            ...
            "🗺️  Auto-Find Stage",
            "⬅️  Back",
        }, nil, "FIND OFFSETS » Auto\n" .. status_bar())

        if     m == 1 then auto_find_gold()
        elseif m == 2 then auto_find_obscured("honor")
        elseif m == 3 then auto_find_obscured("carat")
        elseif m == 4 then auto_find_obscured("soulstone")
        elseif m == 5 then auto_find_obscured("stage")
        else break
        end
    end
end
...
-- ═══════════════════════════════════════════════════════════════
-- 1. FIND OFFSETS  (parent)
-- ═══════════════════════════════════════════════════════════════
local function menu_find_offsets()
    while true do
        local m = gg.choice({
            "🔍 Manual Find  — enter exact in-game value",
            "🤖 Auto Find    — script narrows it down",
            "⬅️  Back",
        }, nil, "FIND OFFSETS\n" .. status_bar())

        if     m == 1 then menu_find_manual()
        elseif m == 2 then menu_find_auto()
        else break
        end
    end
end
...
-- ═══════════════════════════════════════════════════════════════
-- 2a. CURRENCY HACKS → Set Values
-- ═══════════════════════════════════════════════════════════════
local function menu_set_values()
    while true do
        -- Show current last-set values in the label
        local function lbl(key)
            local c = CURR[key]
            local v = c.last > 0 and tostring(c.last) or "—"
            local ok = #c.addrs > 0 and "✅" or "❌"
            return ok .. " " .. c.name .. "  [" .. v .. "]"
        end
        local glbl = (#GOLD.addrs > 0 and "✅" or "❌") ..
                     " Gold  [" .. (GOLD.last > 0 and tostring(GOLD.last) or "—") .. "]"
        ...
        local m = gg.choice({
            ...
            lbl("teamlevel"),
            "⬅️  Back",
        }, nil, "CURRENCY HACKS » Set Values\n" .. status_bar())

        if     m == 1 then set_gold()
        elseif m == 2 then prompt_set("honor")
        elseif m == 3 then prompt_set("carat")
        elseif m == 4 then prompt_set("soulstone")
        elseif m == 5 then prompt_set("stage")
        elseif m == 6 then prompt_set("highstage")
        elseif m == 7 then prompt_set("teamlevel")
        else break
        end
    end
end
...
-- ═══════════════════════════════════════════════════════════════
-- 2b. CURRENCY HACKS → Freeze
-- ═══════════════════════════════════════════════════════════════
local function menu_freeze()
    while true do
        local m = gg.choice({
            "❄️  Freeze All Values",
            "🔓 Unfreeze All",
            "⬅️  Back",
        }, nil, "CURRENCY HACKS » Freeze\n" .. status_bar())

        if     m == 1 then freeze_all()
        elseif m == 2 then unfreeze_all()
        else break
        end
    end
end
...
-- ═══════════════════════════════════════════════════════════════
-- 2. CURRENCY HACKS  (parent)
-- ═══════════════════════════════════════════════════════════════
local function menu_currency()
    while true do
        local m = gg.choice({
            "💸 Set Values   — write amounts to memory",
            "❄️  Freeze       — lock values so they can't drop",
            "⬅️  Back",
        }, nil, "CURRENCY HACKS\n" .. status_bar())

        if     m == 1 then menu_set_values()
        elseif m == 2 then menu_freeze()
        else break
        end
    end
end
...
-- ═══════════════════════════════════════════════════════════════
-- 3a. AUTO HACKS → Auto Farm
-- ═══════════════════════════════════════════════════════════════
local function menu_auto_farm()
    while true do
        local farm_lbl = S.auto_farm
            and "🛑 Stop Auto-Farm  [RUNNING]"
            or  "▶️  Start Auto-Farm [STOPPED]"
        ...
        local m = gg.choice({
            ...
            "🛑 Force Full Auto OFF",
            "⬅️  Back",
        }, nil, "AUTO HACKS » Auto Farm\n" .. status_bar())

        if m == 1 then
            S.auto_farm = not S.auto_farm
            T(S.auto_farm and "🌾 Auto-Farm STARTED" or "🛑 Auto-Farm STOPPED")
        elseif m == 2 then
            S.auto_farm = true
            T("⚡ Full Auto ON")
        elseif m == 3 then
...
-- ═══════════════════════════════════════════════════════════════
-- 3b. AUTO HACKS → Bulk Set (set everything to max in one tap)
-- ═══════════════════════════════════════════════════════════════
local function menu_bulk_set()
    while true do
        local m = gg.choice({
            "💥 MAX Everything  — Gold + all currencies",
            "💰 MAX Gold only",
            "🏅 MAX Honor only",
            "💎 MAX Carat only",
            "👻 MAX Soul Stone only",
            "⬅️  Back",
        }, nil, "AUTO HACKS » Bulk Set\n" .. status_bar())

        if m == 1 then
            -- Gold
            if #GOLD.addrs > 0 then
                local edits = make_edits(GOLD.addrs, 2000000000, GOLD.ftype or T_XOR)
                gg.setValues(edits); GOLD.last = 2000000000
            end
            -- All currencies
            for _, key in ipairs({"honor","carat","soulstone","stage","highstage","teamlevel"}) do
                local c = CURR[key]
                if #c.addrs > 0 then
                    local edits = make_edits(c.addrs, c.max, T_XOR)
                    gg.setValues(edits); c.last = c.max
                end
            end
            T("💥 All maxed!")
            save()
        elseif m == 2 then
            if #GOLD.addrs > 0 then
                local edits = make_edits(GOLD.addrs, 2000000000, GOLD.ftype or T_XOR)
                gg.setValues(edits); GOLD.last = 2000000000; save()
                T("💰 Gold maxed!")
            else T("❌ Find Gold first!") end
        elseif m == 3 then
            if #CURR.honor.addrs > 0 then
                local edits = make_edits(CURR.honor.addrs, CURR.honor.max, T_XOR)
                gg.setValues(edits); CURR.honor.last = CURR.honor.max; save()
                T("🏅 Honor maxed!")
            else T("❌ Find Honor first!") end
        elseif m == 4 then
            if #CURR.carat.addrs > 0 then
                local edits = make_edits(CURR.carat.addrs, CURR.carat.max, T_XOR)
                gg.setValues(edits); CURR.carat.last = CURR.carat.max; save()
                T("💎 Carat maxed!")
            else T("❌ Find Carat first!") end
        elseif m == 5 then
            if #CURR.soulstone.addrs > 0 then
                local edits = make_edits(CURR.soulstone.addrs, CURR.soulstone.max, T_XOR)
                gg.setValues(edits); CURR.soulstone.last = CURR.soulstone.max; save()
                T("👻 Soul Stone maxed!")
            else T("❌ Find Soul Stone first!") end
        else break
        end
    end
end
...
-- 3. AUTO HACKS  (parent)
-- ═══════════════════════════════════════════════════════════════
local function menu_auto_hacks()
    while true do
        local m = gg.choice({
            "🌾 Auto Farm    — keeps values topped up in background",
            "💥 Bulk Set     — one-tap max everything",
            "⬅️  Back",
        }, nil, "AUTO HACKS\n" .. status_bar())

        if     m == 1 then menu_auto_farm()
        elseif m == 2 then menu_bulk_set()
        else break
        end
    end
end
...
-- ═══════════════════════════════════════════════════════════════
-- 4a. CONFIGURATION → Save / Load
-- ═══════════════════════════════════════════════════════════════
local function menu_save_load()
    while true do
        local m = gg.choice({
            "💾 Save Addresses",
            "📂 Load Addresses",
        ...
        elseif m == 2 then
            load()
        elseif m == 3 then
            if gg.alert("Clear ALL saved data?\nThis cannot be undone.", "Yes, Clear", "Cancel") == 1 then
                clear_data()
            end
        elseif m == 4 then
            CLR(); T("🧹 GG results cleared")
        else break
        end
    end
end
...
-- ═══════════════════════════════════════════════════════════════
-- 4b. CONFIGURATION → Status & Info
-- ═══════════════════════════════════════════════════════════════
local function menu_status_info()
    while true do
        local m = gg.choice({
            "📊 Script Status",
            "📐 Offset Info",
        ...
        elseif m == 3 then
            gg.alert(
                "REBIRTH MASTERS v" .. CFG.VER .. " — HELP\n\n" ..
                "STEP 1 — FIND OFFSETS\n" ..
                "  Manual: enter your exact in-game value.\n" ..
                "  Auto:   script narrows down by asking you\n" ..
                "          to enter new values as they change.\n\n" ..
                "STEP 2 — CURRENCY HACKS\n" ..
                "  Set Values: write any number to memory.\n" ..
                "  Freeze: lock so values can't go down.\n\n" ..
                "STEP 3 — AUTO HACKS\n" ..
                "  Auto Farm: runs in background every 500ms.\n" ..
                "  Bulk Set: one tap to max all currencies.\n\n" ..
                "MEMORY TYPES:\n" ..
                "  Honor/Carat/SoulStone/Stage = XOR DWORD\n" ..
                "  Gold = BigInteger (enter full number)\n\n" ..
                "Offsets survive restarts via libil2cpp.so\n" ..
                "relative addressing (v3.4 engine)."
            )
        else break
        end
    end
...
-- ═══════════════════════════════════════════════════════════════
-- 4c. CONFIGURATION → Pointer Chains
-- ═══════════════════════════════════════════════════════════════
local function menu_ptr_chains()
    ptr_chains_menu()   -- existing function handles its own loop
end

-- ═══════════════════════════════════════════════════════════════
-- 4. CONFIGURATION  (parent)
-- ═══════════════════════════════════════════════════════════════
local function menu_configuration()
    while true do
        local m = gg.choice({
            "💾 Save / Load   — persist addresses between sessions",
            "📊 Status & Info — version, offsets, help",
            "🔗 Pointer Chains — register stable pointer paths",
            "⬅️  Back",
        }, nil, "CONFIGURATION\n" .. status_bar())

        if     m == 1 then menu_save_load()
        elseif m == 2 then menu_status_info()
        elseif m == 3 then menu_ptr_chains()
        else break
        end
    end
end
...
-- ═══════════════════════════════════════════════════════════════
-- COMPACT MENU  (shown when minimized / GG reopened)
-- ═══════════════════════════════════════════════════════════════
local function menu_compact()
    local g = #GOLD.addrs > 0 and "✅" or "❌"
    local h = #CURR.honor.addrs > 0 and "✅" or "❌"
    local c = #CURR.carat.addrs > 0 and "✅" or "❌"
    local farm_lbl = S.auto_farm and "🛑 Stop Farm" or "▶️  Start Farm"
    ...
    local m = gg.choice({
        ...
        "RM v%s  G:%s H:%s C:%s\n%s",
        CFG.VER, g, h, c,
        S.auto_farm and "🌾 FARMING" or "Paused"
    ))

    if     m == 1 then S.minimized = false; gg.setVisible(true)
    elseif m == 2 then
        S.auto_farm = not S.auto_farm
        T(S.auto_farm and "▶️ Farm ON" or "🛑 Farm OFF")
    elseif m == 3 then set_gold()
    elseif m == 4 then prompt_set("honor")
    elseif m == 5 then prompt_set("carat")
    elseif m == 6 then gg.setVisible(false); T("📉 Running in background")
    elseif m == 7 then S.running = false
    end
end
...
local function mainMenu()
    local m = gg.choice({
        "1️⃣  Find Offsets      — scan memory for your values",
        "2️⃣  Currency Hacks    — set & freeze gold, gems, etc.",
        "3️⃣  Auto Hacks        — auto farm & bulk max",
        "4️⃣  Configuration     — save/load, offsets, chains",
        "📉 Minimize",
        "❌ Exit",
    }, nil, "⚔️  REBIRTH MASTERS\n" .. status_bar())

    if     m == 1 then menu_find_offsets()
    elseif m == 2 then menu_currency()
    elseif m == 3 then menu_auto_hacks()
    elseif m == 4 then menu_configuration()
    elseif m == 5 then S.minimized = true; gg.setVisible(false); T("📉 Minimized")
    elseif m == 6 then S.running = false
    end
end
...
local function main()
    ...
    if loaded then
        ...
        T("💡 Go to Find Offsets to get started")
    end
